...
build_time_vars = {
    ...
    "CONFIG_ARGS": "'--build=x86_64-unknown-linux-gnu' '--host=aarch64-unknown-linux-gnu' '--prefix=/install' '--with-openssl=/tools/deps' '--with-system-expat' '--with-system-libmpdec' '--without-ensurepip' '--with-readline=editline' '--enable-shared' '--with-mimalloc' '--with-lto' '--with-build-python=/tools/host/bin/python3.13' '--with-dbmliborder=bdb' 'ac_cv_buggy_getaddrinfo=no' 'ac_cv_file__dev_ptc=no' 'ac_cv_file__dev_ptmx=no' 'ac_cv_working_tzset=yes' 'build_alias=x86_64-unknown-linux-gnu' 'host_alias=aarch64-unknown-linux-gnu' 'CC=/usr/bin/aarch64-linux-gnu-gcc' 'CFLAGS= -fPIC  ' 'LDFLAGS=  -Wl,--exclude-libs,ALL -LModules/_hacl' 'CPPFLAGS= -fPIC  '",
    ...
    "PYTHONPATH": "",
    "PYTHON_FOR_BUILD": "_PYTHON_PROJECT_BASE=/build/Python-3.13.1 _PYTHON_HOST_PLATFORM=$(_PYTHON_HOST_PLATFORM) PYTHONPATH=./Lib _PYTHON_SYSCONFIGDATA_NAME=_sysconfigdata__linux_aarch64-linux-gnu _PYTHON_SYSCONFIGDATA_PATH=$(shell test -f pybuilddir.txt && echo /build/Python-3.13.1/`cat pybuilddir.txt`) /tools/host/bin/python3.13",
